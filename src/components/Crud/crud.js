...
function CRUD(options) {
  ...
  console.log(
    "defaultOptions + options(If the properties of dafaultOptions is the same as the properties of options, chiocing the properties of options  preferentially )",
    options
  );
  const data = {
    ...options,
  ...
  console.log("data include the properties of defaultoption and option", data);
  const methods = {
    /**
     * 通用的提示
     */
 ...
 * crud主页
 */
//当调用beforeCreate这时候，还没有混入，data中的数据是当前对象的
function presenter(crud) {
  ...
  return {
    ...
    beforeCreate() {
      ...
      // debugger
      // this.crud.vms[0].vm.crud.title = '用户用户用户用户'
      // console.log(this.crud)
    },
    methods: {
      parseTime,
...
function header() {
  return {
    ...
    beforeCreate() {
      ...
      //由$vnode可知，this表示name为User的组件
      this.crud = lookupCrud(this); //由于presenter方法，这时this有$crud和crud属性
...
function crud(options = {}) {
  ...
  return {
    ...
    beforeCreate() {
      debugger;
      console.log("crud......beforeCreate");
      this.crud = lookupCrud(this);
      this.crud.registerVM(options.type, this);
    },
    destroyed() {
      debugger;
      console.log("crud......destroyed");
